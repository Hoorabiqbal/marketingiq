...
Runs the real FastAPI app (main.app) under uvicorn with two test-only changes:
  - Groq calls are counted and refused, so a load test can never spend provider quota. The
    counter must stay 0.
...
COUNTERS = {"groq_calls": 0, "mock_llm_calls": 0, "entity_index_builds": 0}
